...
**Giải pháp:**
- FE truyền ngôn ngữ qua header `Accept-Language: tw-CN` hoặc `Accept-Language: vi`.
...
- Dữ liệu dịch thuật lưu trong các file JSON:
  ...
  - `app/locales/tw.json` (Tiếng Trung phồn thể)
- Xây dựng hàm `get_message(key, lang)` để lấy text dịch, ví dụ: `get_message("user_not_found", "tw")` -> ``.
...
|---|---|---|
| 1 | Setup i18n | `app/locales/vi.json`, `app/locales/tw.json`, `app/utils/i18n.py` |
